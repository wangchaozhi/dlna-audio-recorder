...
func main() {
	...
	if strings.Contains(port, ":") { parts := strings.Split(port, ":"); port = parts[len(parts)-1] }
	...
	go func() { if err := ssdp.Run(ctx); err != nil { log.Error("SSDP", "err", err); cancel() } }()
	log.Info("DLNA renderer ready", "name", cfg.DeviceName, "advertise_ip", cfg.AdvertiseIP, "output", cfg.OutputDir)
	<-ctx.Done(); rec.Stop()
	shutdownCtx, c := context.WithTimeout(context.Background(), 5*time.Second); defer c(); _ = httpSrv.Shutdown(shutdownCtx)
}
